...
public class Plane implements CSVConvertible {
	 ...
	 * this.model = model; this.planeID = planeID; this.seatMatrix = new
	 * Seat[model.getRow()][model.getColumn()]; // Bunun setterı olmalı mı?
	 ...
	 */
	/*
	 * public void initializeSeats(float regularPrice) { char letter = 'A'; for (int
	 * i = 0; i < model.getColumn(); i++){ for (int ii = 0; ii < model.getRow();
	 * ii++) {
	 * 
	 * Seat s; String currSeatNum = ii + "" + letter;
	 * 
	 * if (ii <= 8) { s = new Seat(currSeatNum, (float) (regularPrice * 1.5),
	 * Seat.SeatClass.BUSINESS); } else { s = new Seat(currSeatNum, regularPrice,
	 * Seat.SeatClass.ECONOMY); }
	 * 
	 * seatMatrix[ii][i] = s; } letter++; } }
	 */
